...
for os in oss:
    ...
    for i, cc_file in enumerate(cc_files):
        ...
        compile_exec = sys.argv[1:] + ["tests/" + cc_file, "-o", exe_file, "-lu", "-L" + obj_dir + "/"]
        compile_process = subprocess.run(compile_exec, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
